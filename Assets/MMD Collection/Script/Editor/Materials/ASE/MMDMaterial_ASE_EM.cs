/*
 * ---------------------------------------------------------------------------
 * Description: Custom material inspector for MMD materials using Amplify Shader Editor (ASE)
 *              variant with Emission Map support.
 ...
 *              alpha clipping, and shadow-related shader keywords.
 * 
...
{
    public class MMDMaterial_ASE_EM : MMDMaterialCustomInspectorBase
    {
        #region === Inspector UI ===
        ...
        protected override void DrawUI()
        {
            ...
            HeaderGroupGUI.DrawGroup(new GUIContent("Emission Map Settings", "Controls the emission (self-illumination) properties of the material."), () =>
            {
                TextureProperty(new GUIContent("Emission Map", "Texture map controlling which areas of the material emit light. White areas emit fully, black areas emit nothing."), properties, "_EmissionMap", showScaleOffset: true);
                ColorProperty(new GUIContent("Emission Color", "Color and intensity of the emitted light. This is combined with the Emission Map to determine the final glow."), properties, "_EmissionColor", hdr: true);
            });
        }
